...
function renderChoices (choices) {
  if (choices.length > 0) { choices.forEach((choice, i) => renderChoice(choice, i)) } else { renderChoice(choices, 0) }
}
/**
...
module.exports = (rl, callback) => {
  network.get_interfaces_list((err, choices) => {
    if (err) global.logger.error('get_interface_list', err)
    ...
    renderChoices(choices)
    rl.on('line', line => {
      if (choices.length > 0) { selected = choices[selected] } else { selected = choices }
      charm.write(`You choosed: ${selected} \r\n`)
      rl.close()
      callback(selected)
    }).on('close', () => {
      global.logger.log('Have a great day!')
      rl.close()
      callback(selected)
    })
  })
